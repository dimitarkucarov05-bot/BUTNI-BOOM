...
export default function Register() {
  ...
  const onRegister = async (e: React.FormEvent) => {
    ...
    if (data.user) {
      // записваме username в profiles; нужен е uniq index за username
      await supabase
        .from('profiles')
        .upsert({ user_id: data.user.id, username }, { onConflict: 'user_id' })
        .then(({ error }) => {
          if (error?.message?.toLowerCase().includes('duplicate') || error?.code === '23505') {
            setErr('Това име е заето. Избери друго.'); return
          }
        })

      // след успешна регистрация → менюто
